...
import { opendirSync, accessSync, constants, mkdirSync, writeFileSync, readFileSync } from 'fs'
import path from 'path/posix'
import moment from 'moment'
import matter from 'gray-matter'

const pan = __dirname.split(':')[0]
const posixPath = path.resolve()

const docsRoot = `${pan}:${posixPath}`; // docs文件路径
    ...
    generateCatalogueFile = async () => {
        ...
        const generateFile = (catalogueMap) => {
            ...
            for (const [dirname, files] of catalogueMap) {
                num++;
                const dirPath = path.join(this.cataloguePath, `0${num}.${dirname}`)
                ...
                for (const file of files) {
                    num2++
                    const filePath = path.join(dirPath, `0${num2}.${file.title}.md`)

                    if (fileExist(filePath)) continue
                    const content = `---\npageComponent: \n name: Catalogue\n data: \n  key: ${file.key}\n  description: ${file.description}\ndate: ${file.date}\ntitle: ${file.title}\npermalink: /0${num2}.${file.permalink}\n---`
...
export default {
    getNavConfig: nav.getNavConfig,
    generateCatalogueFile: nav.generateCatalogueFile
}
// module.exports = {
//     getNavConfig : nav.getNavConfig()
// }
